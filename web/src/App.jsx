      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontWeight: 700, color: C.pAria, fontSize: 16 }}>ARIA</span>
            <span style={{ fontSize: 12, color: C.textDim }}>Assistente de Radiologia por IA</span>
          </div>
...
{ id: "analytics", name: "eX Analytics", tagline: "Dashboards de Produtividade", description: "Exames, produtividade e remuneração em tempo real.", icon: (<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={C.pAnalytics} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M18 20V10"/><path d="M12 20V4"/><path d="M6 20v-6"/></svg>), color: C.pAnalytics, features: ["Tempo real", "Tiers remuneração", "Relatórios auto"] },
...
// ─── Liquid Glass Card ───
function LiquidGlassCard({ pillar, index, inView, onOpenChat }) {
...
return (
<div ref={ref} onMouseEnter={() => setHov(true)} onMouseLeave={() => { setHov(false); setMouse({ x: 0.5, y: 0.5 }); }} onMouseMove={handleMouse} onClick={() => { if (pillar.id === "aria" && onOpenChat) onOpenChat(); }} style={{
...
{pillar.id === "challenge" ? "Aceitar Desafio →" : pillar.id === "aria" ? "Testar ARIA →" : "Explorar →"}
</div>
</div>
</div>
</div>
...
{PILLARS.slice(0, 2).map((p, i) => <LiquidGlassCard key={p.id} pillar={p} index={i} inView={inView} onOpenChat={onOpenChat} />)}
</div>
<div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 18, marginBottom: 18 }}>
{PILLARS.slice(2, 5).map((p, i) => <LiquidGlassCard key={p.id} pillar={p} index={i + 2} inView={inView} onOpenChat={onOpenChat} />)}
</div>
<div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 18, maxWidth: 800, margin: "0 auto" }}>
{PILLARS.slice(5).map((p, i) => <LiquidGlassCard key={p.id} pillar={p} index={i + 5} inView={inView} onOpenChat={onOpenChat} />)}
</div>
</div>
</section>
);
...
<style>{`@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap'); *{box-sizing:border-box;margin:0;padding:0}html{scroll-behavior:smooth}body{background:${C.bg}} @keyframes fadeUp{from{opacity:0;transform:translateY(28px)}to{opacity:1;transform:translateY(0)}} @keyframes gradientShift{0%,100%{background-position:0% 50%}50%{background-position:100% 50%}} @keyframes pulse{0%,100%{opacity:.6}50%{opacity:1}} @keyframes float1{0%,100%{transform:translate(0,0)}50%{transform:translate(30px,-40px)}} @keyframes float2{0%,100%{transform:translate(0,0)}50%{transform:translate(-40px,30px)}} @keyframes float3{0%,100%{transform:translate(0,0)}50%{transform:translate(20px,25px)}} button:hover{filter:brightness(1.08)}a:hover{color:${C.accent}!important} ::-webkit-scrollbar{width:6px}::-webkit-scrollbar-track{background:${C.bg}}::-webkit-scrollbar-thumb{background:${C.border};border-radius:3px} @media(max-width:960px){ #pillars>div>div{grid-template-columns:repeat(2,1fr)!important;max-width:100%!important} #pricing>div>div:nth-of-type(2){grid-template-columns:repeat(2,1fr)!important} #community>div>div:last-child{grid-template-columns:repeat(2,1fr)!important} } @media(max-width:640px){ #pillars>div>div{grid-template-columns:1fr!important} #pricing>div>div:nth-of-type(2){grid-template-columns:1fr!important} #community>div>div:last-child{grid-template-columns:1fr!important} }`}</style>
  ...
  <div style={{ position: "relative", zIndex: 1 }}>
    <Navbar section={sec} />
    <Hero />
    <Pillars onOpenChat={() => setAriaOpen(true)} />
    <Community />
    <Pricing />
    <Embaixadores />
    <Footer />
